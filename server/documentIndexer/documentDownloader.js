...
export default class DocumentDownloader {
  ...
  static download(docName, documentUrl, isIndexable, callback) {
    ...
    fs.remove(downloadPath, (err, res) => {
      if (!err) {
        ...
        console.error(
          "Error while deleting old document files",
          documentUrl,
          err
        );
        callback(err);
    ...
    DocumentDownloader.download(
      ...
      Meteor.bindEnvironment((err, res) => {
        if (!err) {
          ...
          let check = DocumentParser.cleanDocument(
            res.fullPath,
            indexedDocument.url
          );
          ...
          if (!docInfo.indexedBody || docInfo.indexedBody === "") {
            ...
            for (var attrname in docInfo) {
              if (Utils.isEmpty(indexedDocument[attrname]))
                indexedDocument[attrname] = docInfo[attrname];
            }

            let result = Documents.upsert(
              { route: indexedDocument.route },
              indexedDocument
            );
            ...
            function getImg(stats) {
              return (
                stats.isFile() &&
                stats.size > 10240 &&
                (stats.path.indexOf(".jpg") >= 0 ||
                  stats.path.indexOf(".png") >= 0 ||
                  stats.path.indexOf(".jpeg") >= 0)
              );
            }

            readdir(
              directory,
              { filter: getImg, deep: true },
              function (err, files) {
                if (err) {
            ...
            if (result.numberAffected > 0) {
              ...
              console.error(
                "Error while saving document to Database",
                docObj.url,
                errorObj
              );
              callback(errorObj);
  ...
  static fetch(docObj, callback) {
    ...
    if (
      !Utils.isEmptyObject(docObj) &&
      Utils.isString(docObj.docName) &&
      Utils.isString(docObj.url)
    ) {
      console.log("Document URL", docObj.url);

      DocumentDownloader.index(
        docObj,
        Meteor.bindEnvironment((err, res) => {
          if (!err) {
            console.log(
              "Document downloaded and indexed successfully!",
              docObj.url
            );
            callback(null, res);
          } else {
          ...
          if (!err) {
            ...
            for (var attrname in docInfo) {
              if (Utils.isEmpty(document[attrname]))
                document[attrname] = docInfo[attrname];
            }

            console.log(
              "Document downloaded for preview successfully!",
              docObj.url
            );
